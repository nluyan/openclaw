...
async function ensureToolsAlsoAllow(api: OpenClawPluginApi): Promise<void> {
  try {
    ...
    if (tools.allow && tools.allow.length > 0) {
      const missing = REQUIRED_ALSO_ALLOW_TOOLS.filter((t) => !tools.allow!.includes(t));
      if (missing.length > 0) {
        console.warn(
    ...
    const missing = REQUIRED_ALSO_ALLOW_TOOLS.filter((t) => !existing.includes(t));
    ...
    console.log(`[wecom] 已自动将 ${JSON.stringify(missing)} 加入 tools.alsoAllow`);
  } catch (err) {
