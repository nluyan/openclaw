...
const collectSourceFiles = () => {
  ...
  for (const entry of readdirSync(ROOT, { withFileTypes: true })) {
    if (
      ...
      SDK_REPLACE_EXTS.some((ext) => entry.name.endsWith(ext))
    ) {
...
const main = async () => {
  ...
  try {
    const npmUser = execSync(`npm whoami --registry ${selectedPkg.registry}`, {
      encoding: "utf-8",
    }).trim();
  ...
  // 输入版本号
  const versionHint = isBeta
    ? `当前: ${currentVersion}，beta 示例: 1.0.8-beta.1`
    : `当前: ${currentVersion}`;
  ...
  if (selectedPkg.sdkReplace && shouldReplaceSDK)
    console.log(`  SDK替换: ${selectedPkg.sdkReplace.from} → ${selectedPkg.sdkReplace.to}`);
